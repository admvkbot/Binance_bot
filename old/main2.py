...
def check_pipe():
    global path
    return os.path.isfile(path+'pipe2')

def del_pipe():
    global path
    os.remove(path+'pipe2')
...
if not base_price:
    print ("Forgot base_price")
    sys.exit()    

coin = coin + 'USDT'

if(1): 
    stdout=path+'orlovbot2_out.log'
    stderr=path+'orlovbot2_err.log'
    if (1):
        sys.stderr.flush()
...
balance = 1000 # размер позиции для лонга
short_balance = 0 # размер позиции для шорта
#balance = 2 # размер позиции для лонга
#short_balance = 4 # размер позиции для шорта
...
def binance(symbol, base_price):
    ...
    if not long_trigger and balance: 
        ...
        print('Stop price (deactivated): ' + str(stop_price))
        sys.stdout.flush()
        """
